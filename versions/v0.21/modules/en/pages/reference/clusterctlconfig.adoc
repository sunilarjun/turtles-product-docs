...
In order to override a provider version, or to use a different registry to pull images, a `ClusterctlConfig` can be defined. For more information on image override, please refer to the upstream link:https://cluster-api.sigs.k8s.io/clusterctl/configuration#image-overrides[documentation].  
